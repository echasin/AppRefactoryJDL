...
currentAccount: any;
    ...
    loadAll() {
        if (this.currentSearch) {
            this.assetstatusService.search({
    ...
    transition() {
        this.router.navigate(['/assetstatus'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    clear() {
        this.page = 0;
        this.currentSearch = '';
        this.router.navigate(['/assetstatus', {
            page: this.page,
            sort: this.predicate + ',' + (this.reverse ? 'asc' : 'desc')
        }]);
        this.loadAll();
    ...
    trackId(index: number, item: Assetstatus) {
        return item.id;
    }
